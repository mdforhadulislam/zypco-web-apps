    ...
    return async function (req: NextRequest, context?: { params?: any }) {
      ...
      try {
        ...
        if (config.rateLimit || config.customRateLimit) {
          const rateLimiter =
            config.customRateLimit || rateLimiters[config.rateLimit!];
        ...
        if (config.auth?.required !== false) {
          ...
          if (config.auth?.roles?.length) {
            const roleResult = await AuthMiddleware.authorize(
              config.auth.roles
            )(extendedReq);
          ...
          if (config.auth?.permissions?.length) {
            for (const permission of config.auth.permissions) {
              const permResult = await AuthMiddleware.requirePermission(
                permission
              )(extendedReq);
...
export function sanitizeUser(user: any) {
  ...
  const { password, emailVerification, ...sanitized } = user.toObject
    ? user.toObject()
    : user;
  return sanitized;
}
