...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    await connectDB();
    if (!Types.ObjectId.isValid(params.id)) return errorResponse({ status: 400, message: "Invalid ID", req });

    const notification = await Notification.findById(params.id).lean();
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await Notification.findByIdAndUpdate(params.id, updateData, { new: true }).lean();
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const deleted = await Notification.findByIdAndDelete(params.id).lean();
